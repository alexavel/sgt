...
type
  TfrmPneuDim = class(TForm)
    ...
    procedure pdm_codigoExit(Sender: TObject);
    procedure pdm_codigoKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure pdm_codigoRightButtonClick(Sender: TObject);
    procedure ativaget;
    procedure limpaget;
...
procedure TfrmPneuDim.pdm_codigoExit(Sender: TObject);
begin

  // Função que confirma os dados relacionados
  Pesquisa := TClasConsulta.Create;
  Pesquisa.sqlTabela :=  dmEmp.sqlpneudim ;
  pdm_nome.Text :=  Pesquisa.Relacao('pneudim',pdm_codigo,'pdm_codigo','pdm_nome');
  navegacao(5);


end;

procedure TfrmPneuDim.pdm_codigoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 Try
case key of
    VK_END    : pdm_codigoRightButtonClick(Self);
    VK_UP     : navegacao(1);
    VK_LEFT   : navegacao(2);
...
procedure TfrmPneuDim.pdm_codigoRightButtonClick(Sender: TObject);
var
...
if btnGrava.Enabled then
  ...
  Pesquisa.nmTitulo  := 'Tabela de Dimenões de Pneus';
  Pesquisa.nmTabela  := 'pneudim';
  //Pesquisa.NovoCad   := 'pneudim' ;
  // Display        campo   cabecalho           get         tan  indices
  Pesquisa.AddReg('pdm_codigo','Código'             ,'pdm_codigo' ,5  ,'por Código'   );
  Pesquisa.AddReg('pdm_nome'  ,'Dimensão'           ,'pdm_nome'   ,30 ,'por Dimensão' );


  Pesquisa.acessa(pdm_codigo);
  pdm_codigo.text   :=  Pesquisa.RecReg('pdm_codigo');
  pdm_nome.text     :=  Pesquisa.RecReg('pdm_nome');
...
procedure TfrmPneuDim.FormClose(Sender: TObject; var Action: TCloseAction);
begin
    sqlNew.Close;
    dmEmp.clipneudim.Close;
    frmpneudim                   := nil ;
    action                       := cafree;
    fPrincipal.btnpneudim.Enabled := True;
end;



procedure TfrmPneuDim.FormCreate(Sender: TObject);
Var
i: Integer;
begin
  cPre                        := 'pdm_';
  ...
  fPrincipal.btnpneudim.Enabled  := False;
...
procedure TfrmPneuDim.FormKeyPress(Sender: TObject; var Key: Char);
Begin
...
procedure TfrmPneuDim.FormShow(Sender: TObject);
var
i, tmp: Integer;
Pesquisa : TClasConsulta;
begin
  ...
  // Titulos do Banner
  lbCabeca01.Caption    := 'Tabela de Dimenões de Pneus';
...
procedure TfrmPneuDim.gravadados;
Var
foto: TFileStream;
...
SQLQ : TFDQuery ;
begin
   gravadados2(cTabela,cPre,cPre+'codigo','X','', nNew, frmpneudim, dmEmp.SqlConSis, aEdits );
end;

procedure TfrmPneuDim.limpaget;
Var
i: Integer;
begin
  limpaget2(aEdits);
...
procedure TfrmPneuDim.navegacao(Ord: Integer);
begin
  if btnGrava.Enabled then
     Exit;

  navegacao2( Ord, acampo, btnIni, btnAnt, btnProx, btnUlt, sqlNew, dmEmp.sqlpneudim, pdm_codigo,'' );
  acampo := 0;
  RefreshDialog2(cPre,acampo, frmpneudim, sqlNew, aEdits);
  acampo := 1;
end;

procedure TfrmPneuDim.RefreshDados(tipo: String);
var
  i: integer;
  cWhere, cQuery, cOrder: String;

begin
  RefreshDados2(acampo, tipo, '',  pdm_codigo, sqlNew, dmEmp.sqlpneudim );
end;

procedure TfrmPneuDim.RefreshDialog;
var
  i, tmp: Integer;
  frmx : TForm ;
begin
...
procedure TfrmPneuDim.ToolButton9Click(Sender: TObject);
begin
...
procedure TfrmPneuDim.btnAntExecute(Sender: TObject);
begin
  navegacao(2);
end;

procedure TfrmPneuDim.btnCancelaExecute(Sender: TObject);
begin
   acampo := 1;
   nNew   := acampo;
...
procedure TfrmPneuDim.btnExcluirExecute(Sender: TObject);
Var
aSubTab : TArrayDim2;
n: Integer;
begin
   //SetLength(aSubTab, Length(aSubTab)+1); n:= Length(aSubTab)-1; aSubTab[n][0] := 'acessoratv';     aSubTab[n][1] := '';
   If Exclui2(cTabela, aSubTab,  pdm_codigo, btnExcluir) Then Begin;
       RefreshDados('T');
       ativaButoes(1);
       limpaget;
       lMostra := True;
...
procedure TfrmPneuDim.btnGravaExecute(Sender: TObject);
begin
   gravadados;
...
procedure TfrmPneuDim.btnIniExecute(Sender: TObject);
begin
  navegacao(1);
end;

procedure TfrmPneuDim.btnNovoExecute(Sender: TObject);
begin
   acampo := 1;
   nNew   := acampo;
   btnGrava.Enabled := True;
...
procedure TfrmPneuDim.btnProxExecute(Sender: TObject);
begin
  navegacao(3);
end;

procedure TfrmPneuDim.btnSairExecute(Sender: TObject);
begin
  Close;
end;

procedure TfrmPneuDim.btnUltExecute(Sender: TObject);
begin
  navegacao(4);
end;

initialization
  RegisterClass(TfrmPneuDim);
