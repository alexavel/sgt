...
var
  frmRelCadVeiLota: TfrmRelCadVeiLota;
...
procedure TfrmRelCadVeiLota.FormClose(Sender: TObject; var Action: TCloseAction);
begin
    dmEmp.sqlMarca.Close   ;
    frmRelCadVeiLota            := nil    ;
    action                       := cafree ;
end;

procedure TfrmRelCadVeiLota.FormShow(Sender: TObject);
Var
I: Integer;
begin
  // Cor do banner
  ...
  // Titulos do Banner
  lbCabeca01.Caption    := frmRelCadVeiLota.Caption ;
...
for I := 0 to Length( fPrincipal.astatus )-1 do Begin
     cbxStatus.Items.Add(fPrincipal.astatus[I])
End;
...
procedure TfrmRelCadVeiLota.CheckItem(Sender: TObject);
Begin
{}
End;

procedure TfrmRelCadVeiLota.Impressao(nTipo: integer);
Var
...
if cbxStatus.Text <> 'TODOS' then
   cWhere := ' where vat_status = '+QuotedStr( cbxStatus.Text );

if ( cbxStatus.Text <> 'TODOS' ) and ( edCodigo.Text <> '' ) then
   cWhere := ' where vat_codltd = '+edCodigo.Text+' and vat_status = '+QuotedStr( cbxStatus.Text );

  { Formulando o tipo de impressao }
  cTitulo := 'LISTAGEM DE VEÍCULOS :'+fIIF( edCodigo.Text ='',' LOTADO ',' POR LOTAÇÃO: '+Trim( edCodigo.Text)
                                    +' - '+Trim(edNomCon.Text ) )+' ORDENADO POR '+U_funcao.fIIF( rbAlfa.Checked, 'DESCRIÇÃO','CÓDIGO' )
                                    +' - '+UpperCase( cbxStatus.Text );
  ...
  with sqlTab do
     begin
       ...
       Sql.Text :='Select * from  '+U_funcao.SelSchema('veiculoatv')
                ...
                +' '+cWhere
                +' order by vat_codltd,  '+fIIF(rbAlfa.Checked, ' vat_nrfrota ', ' vat_placa ' );
       Open;
  end;

  // Cabeçalho do relatorio
  CabReports( frxReportTab, cTitulo, 'RelCadVeiculoLota.fr3' );
...
procedure TfrmRelCadVeiLota.edCodigoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 Try
case key of
...
procedure TfrmRelCadVeiLota.edCodigoRightButtonClick(Sender: TObject);
var
...
begin
  ...
  edNomCon.text   :=  Pesquisa.RecReg('ltd_nome');
  Pesquisa.Destroy;

end;

procedure TfrmRelCadVeiLota.mnImprimeExecute(Sender: TObject);
begin
 Impressao(2);
end;

procedure TfrmRelCadVeiLota.mnSairExecute(Sender: TObject);
begin
  Close;
end;

procedure TfrmRelCadVeiLota.mnVizualExecute(Sender: TObject);
begin
 Impressao(1);
end;
