...
type
  TfrmTpContainer = class(TForm)
    ...
    procedure tcn_codigoExit(Sender: TObject);
    procedure tcn_codigoKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure tcn_codigoRightButtonClick(Sender: TObject);
    procedure ativaget;
    procedure limpaget;
...
var
  frmTpContainer: TfrmTpContainer;
...
procedure TfrmTpContainer.tcn_codigoExit(Sender: TObject);
begin
  ...
  tcn_nome.Text :=  Pesquisa.Relacao('tpcontainer',tcn_codigo,'tcn_codigo','tcn_nome');
  navegacao(5);
end;

procedure TfrmTpContainer.tcn_codigoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 Try
case key of
    VK_END    : tcn_codigoRightButtonClick(Self);
    VK_UP     : navegacao(1);
    VK_LEFT   : navegacao(2);
...
procedure TfrmTpContainer.tcn_codigoRightButtonClick(Sender: TObject);
var
...
if btnGrava.Enabled then
  ...
  Pesquisa.nmTitulo  := 'Tabela de Tipos de Container';
...
//  Pesquisa.NovoCad   := 'tpcontainer' ;
  ...
  Pesquisa.AddReg('tcn_nome'    ,'Serviço'                ,'tcn_nome'   ,30 ,'por Serviço'      );


  Pesquisa.acessa(tcn_codigo);
  tcn_codigo.text   :=  Pesquisa.RecReg('tcn_codigo');
  Pesquisa.Destroy;
...
procedure TfrmTpContainer.FormClose(Sender: TObject; var Action: TCloseAction);
begin
    sqlNew.Close;
    dmEmp.clitpcontainer.Close;
    frmtpcontainer                 := nil ;
    action                       := cafree;
    fPrincipal.btntpcontainer.Enabled := True;
end;



procedure TfrmTpContainer.FormCreate(Sender: TObject);
Var
i: Integer;
begin
  cPre                        := 'tcn_';
  ...
  fPrincipal.btntpcontainer.Enabled  := False;
...
procedure TfrmTpContainer.FormKeyPress(Sender: TObject; var Key: Char);
Begin
...
procedure TfrmTpContainer.FormShow(Sender: TObject);
var
i, tmp: Integer;
Pesquisa : TClasConsulta;
begin
  ...
  // Titulos do Banner
  lbCabeca01.Caption    := 'Tabela de Tipos de Containers';
...
procedure TfrmTpContainer.gravadados;
Var
foto: TFileStream;
...
begin
   gravadados2(cTabela,cPre,cPre+'codigo','X','', nNew, frmtpcontainer, dmEmp.SqlConSis, aEdits );
end;

procedure TfrmTpContainer.limpaget;
Var
i: Integer;
begin
  limpaget2(aEdits);
...
procedure TfrmTpContainer.navegacao(Ord: Integer);
begin
  if btnGrava.Enabled then
     Exit;

  navegacao2( Ord, acampo, btnIni, btnAnt, btnProx, btnUlt, sqlNew, dmEmp.sqltpcontainer, tcn_codigo,'' );
  acampo := 0;
  RefreshDialog2(cPre,acampo, frmtpcontainer, sqlNew, aEdits);
  acampo := 1;
end;

procedure TfrmTpContainer.RefreshDados(tipo: String);
var
  i: integer;
  cWhere, cQuery, cOrder: String;

begin
  RefreshDados2(acampo, tipo, '', tcn_codigo, sqlNew, dmEmp.sqltpcontainer );
end;

procedure TfrmTpContainer.RefreshDialog;
var
  i, tmp: Integer;
  frmx : TForm ;
begin
...
procedure TfrmTpContainer.ToolButton9Click(Sender: TObject);
begin
...
procedure TfrmTpContainer.btnAntExecute(Sender: TObject);
begin
  navegacao(2);
end;

procedure TfrmTpContainer.btnCancelaExecute(Sender: TObject);
begin
   RollBack;
   acampo := 1;
   nNew             := acampo;
...
procedure TfrmTpContainer.btnExcluirExecute(Sender: TObject);
Var
aSubTab : TArrayDim2;
n: Integer;
begin
   //SetLength(aSubTab, Length(aSubTab)+1); n:= Length(aSubTab)-1; aSubTab[n][0] := 'acessoratv';     aSubTab[n][1] := '';
   If Exclui2(cTabela, aSubTab,  tcn_codigo, btnExcluir) Then Begin;
       RefreshDados('T');
       ativaButoes(1);
       limpaget;
       lMostra := True;
...
procedure TfrmTpContainer.btnGravaExecute(Sender: TObject);
begin
   gravadados;
...
procedure TfrmTpContainer.btnIniExecute(Sender: TObject);
begin
  navegacao(1);
end;

procedure TfrmTpContainer.btnNovoExecute(Sender: TObject);
begin
   acampo := 1;
   nNew   := acampo;
   btnGrava.Enabled := True;
...
procedure TfrmTpContainer.btnProxExecute(Sender: TObject);
begin
  navegacao(3);
end;

procedure TfrmTpContainer.btnSairExecute(Sender: TObject);
begin
  Close;
end;

procedure TfrmTpContainer.btnUltExecute(Sender: TObject);
begin
  navegacao(4);
end;

initialization
  RegisterClass(TfrmTpContainer);
