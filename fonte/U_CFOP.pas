...
type
  TfrmCfop = class(TForm)
    ...
    procedure cfo_codigoExit(Sender: TObject);
    procedure cfo_codigoKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure cfo_codigoRightButtonClick(Sender: TObject);
    procedure ativaget;
    procedure limpaget;
...
procedure TfrmCfop.cfo_codigoExit(Sender: TObject);
begin
  if lMostra then begin
      // Função que confirma os dados relacionados
      Pesquisa := TClasConsulta.Create;
      Pesquisa.sqlTabela :=  dmEmp.sqlcfop ;
      cfo_nome.Text :=  Pesquisa.Relacao('cfop',cfo_codigo,'cfo_codigo','cfo_nome');
      navegacao(5);
  end;
end;

procedure TfrmCfop.cfo_codigoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 Try
case key of
    VK_END:
      cfo_codigoRightButtonClick(Self);
    VK_UP     : navegacao(1);
    VK_LEFT   : navegacao(2);
...
procedure TfrmCfop.cfo_codigoRightButtonClick(Sender: TObject);
var
...
if btnGrava.Enabled then
  ...
  Pesquisa.AddReg('cfo_codigo','Código'    ,'cfo_codigo' ,5   ,'por Código'    );
  Pesquisa.AddReg('cfo_nome'  ,'Descrição' ,'cfo_nome'   ,100 ,'por Descrição' );

  Pesquisa.acessa(cfo_codigo);
  cfo_codigo.text   :=  Pesquisa.RecReg('cfo_codigo');
  Pesquisa.Destroy;

  if Pesquisa.lOk then Begin
     cfo_nome.text     :=  Pesquisa.RecReg('cfo_nome');
     acampo             := 0;
     RefreshDados('S');
...
begin
    ...
    frmcfop                    := nil ;
    action                       := cafree;
    fPrincipal.btncfop.Enabled   := True;
end;
...
begin
  ...
  fPrincipal.btncfop.Enabled  := False;
...
begin
  ...
  // Titulos do Banner
  lbCabeca01.Caption    := 'Tabela de CFOPs';
...
SQLQ : TFDQuery ;
begin
   gravadados2(cTabela,cPre,cPre+'codigo','X','', nNew, frmcfop, dmEmp.SqlConSis, aEdits );
end;

procedure TfrmCfop.limpaget;
Var
i: Integer;
begin
  limpaget2(aEdits);
...
procedure TfrmCfop.navegacao(Ord: Integer);
begin
  if btnGrava.Enabled then
     Exit;

  navegacao2( Ord, acampo, btnIni, btnAnt, btnProx, btnUlt, sqlNew, dmEmp.sqlcfop, cfo_codigo,'' );
  acampo := 0;
  RefreshDialog2(cPre,acampo, frmcfop, sqlNew, aEdits);
  acampo := 1;
end;


procedure TfrmCfop.RefreshDados(tipo: String);
var
  i: integer;
  cWhere, cQuery, cOrder: String;

begin
  RefreshDados2(acampo, tipo, '', cfo_codigo, sqlNew, dmEmp.sqlcfop );
end;

procedure TfrmCfop.RefreshDialog;
var
  i, tmp: Integer;
  frmx : TForm ;
begin
...
procedure TfrmCfop.ToolButton9Click(Sender: TObject);
begin
...
procedure TfrmCfop.btnAntExecute(Sender: TObject);
begin
  navegacao(2);
end;

procedure TfrmCfop.btnCancelaExecute(Sender: TObject);
begin
   acampo := 1;
   nNew             := acampo;
   ...
   if cfo_codigo.Enabled then
      cfo_codigo.SetFocus;

end;

procedure TfrmCfop.btnExcluirExecute(Sender: TObject);
Var
aSubTab : TArrayDim2;
n: Integer;
begin
   //SetLength(aSubTab, Length(aSubTab)+1); n:= Length(aSubTab)-1; aSubTab[n][0] := 'acessoratv';     aSubTab[n][1] := '';
   If Exclui2(cTabela, aSubTab,  cfo_codigo, btnExcluir) Then Begin;
       RefreshDados('T');
       ativaButoes(1);
       limpaget;
       lMostra := False;
...
procedure TfrmCfop.btnGravaExecute(Sender: TObject);
begin
   gravadados;
...
procedure TfrmCfop.btnIniExecute(Sender: TObject);
begin
  navegacao(1);
end;

procedure TfrmCfop.btnNovoExecute(Sender: TObject);
begin
   acampo := 1;
   nNew   := acampo;
   btnGrava.Enabled := True;
...
procedure TfrmCfop.btnProxExecute(Sender: TObject);
begin
  navegacao(3);
end;

procedure TfrmCfop.btnSairExecute(Sender: TObject);
begin
  Close;
end;

procedure TfrmCfop.btnUltExecute(Sender: TObject);
begin
  navegacao(4);
end;

initialization
  RegisterClass(TfrmCfop);
